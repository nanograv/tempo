...
c	compute precession matrix (corresponding to transpose of original prcnut version).
	call precession(fmjd, prc, dummy)
c	compute nutation matrix (corresponding to transpose of original prcnut version).
...
c		equinox of date to J2000.
		prn(j,i) = nut(i,1)*prc(1,j) + nut(i,2)*prc(2,j) + nut(i,3)*prc(3,j)
	enddo
	enddo
	end
