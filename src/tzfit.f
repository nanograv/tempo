c      $Id$
	...
	i=i+1
	t(j)=tmin(i)-rtime
10	ph(j)=phase(i)-rphase-t(j)*f0*60.d0	
	fac=2.d0/nn
	do 13 j=1,nn
	sum=0.d0
	do 12 k=1,nn
12	sum=sum+ph(k)*cos((pi*(j-1))*((k-0.5d0)/nn))
13	c(j)=fac*sum
...
1070	format(a10,1x,a9,f11.2,f20.11,f21.6,1x,f6.3,f7.3/
